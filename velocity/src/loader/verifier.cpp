...
namespace spade
{
    ...
    void Verifier::check_class(const ClassInfo &klass, const uint16 cp_count) {
        if (klass.kind > 0x03)
            throw corrupt();
    ...
    void Verifier::check_field(const FieldInfo &field, const uint16 cp_count) {
        if (field.kind > 0x01)
            throw corrupt();
    ...
    void Verifier::check_method(const MethodInfo &method, const uint16 cp_count) {
        ...
        const uint32 code_count = method.code_count;
        check_line(method.line_info, code_count);
        for (const auto &match: method.matches) check_match(match, code_count, cp_count);
    }

    void Verifier::check_match(const MatchInfo &info, const uint32 code_count, const uint16 cp_count) {
    ...
    void Verifier::check_line(const LineInfo &line, const uint32 code_count) {
    ...
    void Verifier::check_global(const GlobalInfo &global, const uint16 cp_count) {
        if (global.kind > 0x01)
            throw corrupt();
