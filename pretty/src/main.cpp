    ...
    class PrettyDebugger : public Debugger {
      ...
      public:
        ...
        void init(const SpadeVM *) {
            Initialize(state);
            cmds.add_command("quit", [this]() { CloseWindow(state); });
            cmds.add_command("clear", [this]() { console = {""}; });
        }
            ...
            BeginGridCell(state, 2, 1); {
                ...
                TextField(state, command_line, {
                    ...
                    .on_enter = [&](TextFieldInfo &) {
                        ...
                        println(command);
                        const auto cmd_map = cmds.get_command(command);
                        if (cmd_map.size() == 1)
                            cmd_map.begin()->second();
                    },
                });
                EndBorder(state);
                // The output
      ...
      private:
            ...
            while (i < code_count) {
                ...
                case 1: {
                    ...
                    if (OpcodeInfo::take_from_const_pool(opcode))
                        param = std::format("{} ({})", num, sanitize_text(pool[num].to_string()));
                    else
                        param = std::to_string(num);
                    break;
                }
                case 2: {
                    ...
                    case Opcode::JGE:
                    case Opcode::JGT:
                        param = std::to_string(static_cast<int16_t>(num));
                        break;
                    default:
                        if (OpcodeInfo::take_from_const_pool(opcode))
                            param = std::format("{} ({})", num, sanitize_text(pool[num].to_string()));
            ...
            for (size_t i = 0; i < instructions.size(); i++) {
                ...
                RichTextBox(state, {
                    .text = color_fmt("%({},#ffffff){} %({},#ffb626){: >{}}%({},#ffffff): {} %({},#82ff9e){}%({},#ffffff) {}", 
                                line_bg, " ",
                                line_bg, instr.start, byte_line_max_len, line_bg,
                                instr.source_line_str, 
                                line_bg, OpcodeInfo::to_string(instr.opcode), line_bg,
                                instr.param),
                    .pos = {0, i},
        ...
        void OperandStack(const Frame *frame) {
            ...
            for (size_t i = frame->get_args_count() + frame->get_locals_count(); i < frame->sc; i++) {
                ...
                const auto text = " " + sanitize_text(value.to_string());
                data.push_back(text);
            }
            // clang-format off
        if (data.empty()) {
        ...
        void ArgsTable(const Frame *frame) {
            ...
            for (uint8_t i = 0; i < frame->get_args_count(); i++) {
                ...
                data.push_back(sanitize_text(frame->get_arg(i).to_string()));
            }
        ...
        void LocalsTable(const Frame *frame) {
            ...
            for (uint8_t i = 0; i < frame->get_locals_count(); i++) {
                data.push_back(std::to_string(i));
                data.push_back(sanitize_text(frame->get_local(i).to_string()));
            }
        ...
        void CallStack(const ThreadState &th_state) {
            ...
            for (auto frame = th_state.get_frame(); frame >= call_stack; frame--) {
                ...
                table.push_back(sanitize_text(frame->get_method()->to_string()));
            }
...
int main(int argc, char **argv) {
    ...
    auto debugger = std::make_unique<pretty::PrettyDebugger>();
